...
class XQAsynHttp: NSObject {
    ...
    class func sendGetHttp(url : String,isNSUTF8StringEncoding : Bool) -> NSURLRequest{
        ...
        var str : String = "http://\(self.IP)/shopApp/\(url)"
    ...
    class func sendPostHttp(url : String ,var strHTTPBody : String,isNSUTF8StringEncoding : Bool) {
        
        //1.URL
        let url : NSURL = NSURL(string: "http://\(self.IP)/shopApp/\(url)")!
