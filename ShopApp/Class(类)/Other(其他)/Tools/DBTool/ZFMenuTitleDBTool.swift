...
class ZFMenuTitleDBTool: NSObject {
...
//            println("t_menuTitle表创建成功")
    ...
    class func insertMenuTitle(db : FMDatabase,menuTitle : ZFMenuTitleModel){
...
//            println("menuTitle删除成功")
    ...
    class func selectMenuTitleAll(db : FMDatabase) -> Array<ZFMenuTitleModel>{
        ...
        while set.next() {
            ...
            let menuTitle = ZFMenuTitleModel()
            
            menuTitle.menuTitle_id = Int(set.intForColumn("menuTitle_id"))
            menuTitle.menuTitle_name = set.stringForColumn("menuTitle_name")
            
            menuTitle.menuContents = ZFMenuContentDBTool.selectMenuContent(db, menuTitle_id: menuTitle.menuTitle_id! as Int)
